...
fn main() {
    ...
    let tautomer_enumerator_result = rdkit_sys::mol_standardize_ffi::tautomer_enumerate(tautomer_enumerator.clone(), mol.clone());
    ...
    let second = rdkit_sys::mol_standardize_ffi::tautomer_enumerator_result_tautomers_at(tautomer_enumerator_result.clone(), 1);
    let second_smile = rdkit_sys::ro_mol_ffi::mol_to_smiles(second);
    println!("second smile: {}", second_smile);

    let canonical_mol = rdkit_sys::mol_standardize_ffi::tautomer_enumerator_canonicalize(tautomer_enumerator.clone(), mol.clone());
